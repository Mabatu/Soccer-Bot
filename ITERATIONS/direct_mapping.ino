...
void processGamepad(ControllerPtr ctl) {
  ...
  if (LY < NEUTRAL_POINT - DEAD_ZONE) {
    ...
    analogWrite(rightWheels, -1 * (LY - 4) * 2);
    analogWrite(leftWheels, (-1 * LY - 4) * 2);
  }
    ...
    analogWrite(leftWheels, (LY - 4) * 2);
    analogWrite(rightWheels, (LY - 4) * 2);
  }
  ...
  if (RX > NEUTRAL_POINT + DEAD_ZONE) {
    ...
    analogWrite(rightWheels, (RX - 4) * 2);
    analogWrite(leftWheels, (RX - 4) * 2);
  }
    ...
    analogWrite(leftWheels, -1 * (RX - 4) * 2);
    analogWrite(rightWheels, -1 * (RX - 4) * 2);
  }
  if (L2 > 4) {
    digitalWrite(RightWheelsForward, LOW);
    digitalWrite(RightWheelsReverse, HIGH);
